...
export const uploadImagesCloudinaryApiRequest = (images, productId) => {
    const url = "https://api.cloudinary.com/v1_1/dxvn6eqee/image/upload"
    const formData = new FormData()
    for (let i = 0; i < images.length; i++) {
        let file = images[i]
        formData.append("file", file)
        formData.append("upload_preset", "y8oekm5d") //el 2do viene de la página de cloudinary, en la parte de "upload"
        fetch(url, {
            method: "POST",
            body: formData,
        })
